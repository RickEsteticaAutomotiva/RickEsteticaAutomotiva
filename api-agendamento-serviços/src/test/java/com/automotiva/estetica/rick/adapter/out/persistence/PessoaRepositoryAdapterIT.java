 ...
 *
 * <p>
 * Sobe apenas a camada JPA (sem servidor web), usando H2 em memória. Valida
 * queries, specifications e o mapeamento Entity ↔ Domain Object.
 */
@DataJpaTest
@ActiveProfiles("test")
...
class PessoaRepositoryAdapterIT {
     ...
     *
     * <p>
     * O campo {@code roles} é deixado com o valor default ({@code new HashSet<>()})
     * porque o {@code TestEntityManager} opera fora do
     * {@link PessoaRepositoryAdapter}, que é quem resolve as {@link RoleJpaEntity}
     * pelo {@link RoleJpaRepository}. Para testes que não exercitam roles, isso é
     * suficiente.
     */
    private PessoaJpaEntity criarPessoaJpa(String nome, String cpf, String email) {
        return entityManager.persistFlushFind(PessoaJpaEntity.builder().nome(nome).cpf(cpf).email(email)
     ...
     *
     * <p>
     * Cada {@link RoleEnum} informado é primeiro persistido como
     * {@link RoleJpaEntity} (ou buscado, se já existir), garantindo integridade
     * referencial na tabela {@code pessoa_roles} sem depender do
     * {@link PessoaRepositoryAdapter}.
     */
    private PessoaJpaEntity criarPessoaJpaComRoles(String nome, String cpf, String email, EnumSet<RoleEnum> roles) {
        Set<RoleJpaEntity> roleEntities = roles.stream().map(roleEnum -> {
            RoleJpaEntity role = RoleJpaEntity.builder().nome(roleEnum).build();
            return entityManager.persistFlushFind(role);
        }).collect(Collectors.toSet());

        return entityManager
                .persistFlushFind(PessoaJpaEntity.builder().nome(nome).cpf(cpf).email(email).telefone("11999990001")
    ...
    void salvar_sucesso() {
        Pessoa pessoa = Pessoa.builder().nome("Novo Usuário").cpf("11122233344").email("novo@email.com")
    ...
    void deletarPorId_sucesso() {
        ...
        // Hibernate.
        // entityManager.find() e qualquer método JPA derivado aplicam o filtro
        // automaticamente,
    ...
    void salvar_semRoles_devePersistirRoleUserDefault() {
        ...
        Pessoa pessoa = Pessoa.builder().nome("Usuário Padrão").cpf("12312312312").email("padrao@email.com")
