...
class VeiculoControllerIT extends AbstractIntegrationTest {
    ...
    void buscarPorPessoa_sucesso() throws Exception {
        mockMvc.perform(
                        get(BASE_PATH + "/veiculos/pessoa/1")
    ...
    void buscarPorPessoa_listaVazia() throws Exception {
        ...
        mockMvc.perform(
                        get(BASE_PATH + "/veiculos/pessoa/9999")
                                .header("Authorization", bearer(tokenAdmin)))
                .andExpect(status().isOk())
    ...
    void deletar_sucesso() throws Exception {
        mockMvc.perform(
                        delete(BASE_PATH + "/veiculos/3")
                                .header("Authorization", bearer(tokenAdmin)))
                .andExpect(status().isNoContent());

        // Confirma que o veículo não aparece mais na listagem da pessoa (@SQLRestriction filtra
    ...
    void deletar_naoEncontrado() throws Exception {
        mockMvc.perform(
                        delete(BASE_PATH + "/veiculos/9999")
                                .header("Authorization", bearer(tokenAdmin)))
